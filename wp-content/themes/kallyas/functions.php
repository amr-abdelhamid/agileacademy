...
--------------------------------------------------------------------------------------------------*/
	...
	define( 'OPTIONS', 'zn_kallyas_options' ); 
 	...
 	update_option('siteurl', HOME_URL); //'http://www.agileacademy.co/');
 	update_option('home', HOME_URL); //'http://www.agileacademy.co/');
		...
		if ($data['face_og']) { ?>
			...
			<script>
				(function (d, s, id) {
					...
					js.id = id;
					js.src = "//connect.facebook.net/en_US/all.js#xfbml=1&appId=<?php echo $data['face_AP_ID'];?>"; // addyour appId here
...
add_image_size( 'event-feature-image', 300, 9999 ); //300 pixels wide (and unlimited height)
