  ...
  public function get_thumbnail($single, $lcp_thumb_class = null){
    ...
    if ($this->params['thumbnail']=='yes'):
      ...
      if ( has_post_thumbnail($single->ID) ):
        ...
        /* Amr Noaman - don't resize the image
		...
		$image = wp_get_attachment_image_src( get_post_thumbnail_id( $single->ID ), 'full' );
		$img_url = $image[0]; // get the src of the featured image
		$lcp_thumbnail .= '<img src="'.$img_url.'" width="'.$lcp_thumb_size[0].'" alt="'.esc_attr($single->post_title).'" />'; // not using height="'.$image_height.'" because images could be not square shaped and they will be stretched
